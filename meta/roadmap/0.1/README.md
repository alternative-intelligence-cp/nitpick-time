...
> 0.1.5.**
>
> **`0.1.3c.md` was written by a seventh planner on 2026-09-26, after 0.1.4c's
> close, against compiler `c970483`** — rehearsed from its own fenced blocks
> first in fresh clones beside a linked workbench, then in the REAL checkout on
> its own commit, the tree put back to that commit afterwards; its last
> section says which command ran where. **It found two things at planning**:
> that the marker which makes a `Vec` move-only also makes a by-value `Vec`
> parameter a read-only loan at this pin, so `vec_at` must take a loan to read
> one (`0.1.3c.md` §1.5); and, by the reason-sweep the orchestrator asked for,
> one live rule still stating a discharged defect as current, `SAFETY.md`
> §1's move-only row (§6.2).
...
- ~~**`../../OPEN_QUESTIONS.md` Q-6** — what replaces `VERIFICATION.md` P-1 now
  ...
  P-1b (TM-197); Q-6 is struck with that decision's number.**
...
| 0.1.3c | **`Vec` move-only by construction** — question 9, answered 2026-09-25 — porting `nitpick-regex`'s 0.0.4d and 0.0.4e, both landed and verified: the marker field; `vec_at<T: Pod>` through a `Pod` trait and a LOAN, which the marker forces at this pin, and `generic_owning_copy/case5` refused `NITPICK-TYPE-017`; `Bytes.body` hidden, which question 9's answer let ride; TM-150's churn pair committed with `heap:` bounds; A′ as `VERIFICATION.md` rule P-1b, striking Q-6; `Pod` shaped for the prelude's `Copy` to replace at a later re-pin; and the reason-sweep over the six defects the registry struck at `c970483` — **[`0.1.3c.md`](0.1.3c.md), DONE 2026-09-26, against compiler `c970483`; it ran after 0.1.4c** (the orchestrator, 2026-09-26) | no copy of a `Vec` and no destructive `vec_at` compiles (TM-193, TM-194); `GREEN -- 112 unit(s)`, and CI green — run `36245367277` (TM-193 … TM-198) |
...
### 0.1.3c — `Vec` move-only by construction — [`0.1.3c.md`](0.1.3c.md) §9 is the full list — **DONE 2026-09-26**
- [x] `Vec` move-only by construction, porting `nitpick-regex`'s 0.0.4d: `hidden string[0]:move_only`, and a copy of a `Vec`, an assignment of one over another and a copy of a struct holding one each refused `NITPICK-TYPE-046` — each run to the free poison against the tree before, and against an `int64[0]` marker (TM-193) — at execution, `facts.py`'s parts D and E equal to the plan's: the marker alone moving none of the 116 files, the three copies refused at both pins and `run:70` on both legs against the library before and over `int64[0]`; `mut.py`'s rows A and B red on each, by their named lines
- [x] `vec_at<T: Pod>` through a loan, porting its 0.0.4e — `Pod` and its nine impls exactly `nitpick-regex`'s; `vec_pop` already spells `move`; and `vec_at`'s `#wild_slice` read at an owning `T`, which DEF-104's gate does not reach (`0.1.4c.md` §1.3, §11), settled: `case5` refused `NITPICK-TYPE-017`, its three worlds in its transcript (TM-194) — at execution, `Pod`'s eleven lines identical to `nitpick-regex`'s at its `fb37391`; the loan table, the three owner impls refused `TYPE-047` and the hole at 95 re-measured; the transcript's six rows 6 of 6; `case5` refused `TYPE-017` locally and in CI
- [x] `Bytes.body` hidden and `bytes_capacity` — question 9's answer, judged cheaper here (TM-195) — at execution the sixteen `TYPE-080`s before the tests moved (11, 4 and 1), `probe16i` refused `TYPE-080` and red against the library before and against `body` sealed again; `bytes_growth` and `bytes_view_lifetime` at 2 048 636 and 160, as they were
- [x] **from 0.1.4b's planning:** TM-150's `vec_pop` churn pair — two million push-then-pop cycles, `peak_live` 120 against `vec_clear`'s 48 000 096 — committed as two programs with `heap:` bounds and the cap, every bound seen red on its mutant (TM-196; `0.1.4b.md` §15) — at execution 120 and 48 000 096, `count` 2 000 001, on both legs at both pins and in CI; 0 and 92 under the cap; `mut.py`'s K1 … K4 red by their named lines, D1 fooled, and the ceiling's rule recomputed at 75 000
- [x] A′ as `VERIFICATION.md` rule P-1b, and Q-6 struck with its number (TM-197) — at execution, a live `requires` on `vec_at` re-measured at 116 against the slice's 94, and ten identities against nine
- [x] `Pod` shaped so the prelude's `Copy` — the compiler's D-327, ratified during this planning and in no pin of ours — replaces it mechanically at the re-pin that carries it, here and in `nitpick-regex`; nothing named `Copy` (TM-198) — at execution, `Pod` in one block of `src/core/vec.npk`, one bound, one call, one umbrella line and three test impls, and no code line anywhere naming `Copy`
- [x] the reason-sweep over the six defects the registry struck at `c970483` (`0.1.3c.md` §6.2) — every site classified, and the one live rule it found corrected — at execution 206 lines over 32 files before and 207 after, each pass equal to its block; `SAFETY.md` §1's row corrected, and pass P's hit on it gone
- [x] `GREEN -- 112 unit(s)` locally and in CI — the work commit `0f588b9`; 132.4 s locally, and CI run `36245367277`, read from its own log, 0 differences
- [x] **found at execution, not planned:** two sentences the marker made inexact, which neither of the plan's sweeps names — `SAFETY.md`'s *"Where `List<T>` is"* paragraph (*"three fields"*, *"gets none of that"*) and `probe06_generic_vec.npk`'s *"B-12's shape, verbatim"* — found by a second sweep of 153 candidates and carried to 0.1.5's row
