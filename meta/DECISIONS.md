...
# Cycle 0.1.2 — the sweep, ratified 2026-09-25

Four decisions, drafted at planning (`meta/roadmap/0.1/0.1.2.md` §12, PD-16 …
PD-19, in that order) and recorded here by the worker in the commit that makes
the change each describes. Every measurement is at compiler `c3bdae2` and is
recorded with its command in `0.1.2.md`'s execution record; the ones taken at
planning were re-run at execution and matched.

### TM-165 — the weekday cycle, C-17's second rider, is asserted from cycle 0.1.3, where `weekday()` is written; 0.1.2 asserts the `+ 1` chain it rests on

**2026-09-25, cycle 0.1.2 (PD-16).** C-17's second rider — *"the weekday
advances by exactly one, modulo seven, per day"* — is a property of a weekday
FUNCTION, and `src/cal/` has none until cycle 0.1.3: `weekday_number` maps a
`Weekday` to a number, and the weekday OF A DATE is the first item of the cycle
README's 0.1.3 checklist. The original plan put the rider one subcycle before
its subject.
...
consecutive dates have consecutive day numbers — which is
`tests/unit/sweep/every_civil_date.npk`'s `+ 1` chain, asserted at every one of
the range's 7 304 483 steps, every century and every 400-year boundary among
them (TM-167). The rider joins that walk at 0.1.3, compared against a count
begun at −9999-01-01's **Monday** — re-derived at execution without Hinnant:
Python's `date(1, 1, 1)` and `date(401, 1, 1)` are Mondays, reached from
−9999-01-01 by 25 and 26 whole 400-year cycles, and 146 097 days is exactly
20 871 weeks.

*Cost:* the cycle's gate completes at 0.1.3, not 0.1.2 — `CALENDAR.md` C-16 and
two of C-17's three riders here, the third with the function it asserts.

*Declined:* `weekday()` pulled into 0.1.2 — it would design one derived field
apart from the three it is designed with (C-13, C-14, C-15), and take a public
name (TM-013) a subcycle early; a weekday computed in the test from the day
number — implied by the chain everywhere, so one method and not two, and a
check of test code rather than library code; the rider dropped — C-17 names
it.

### TM-166 — three `sweep` members, one domain each, under a `[[test]] sweep` entry; `for` loops; the print helper inline in each

**2026-09-25, cycle 0.1.2 (PD-17).** `nitpick.toml` gains `[[test]] sweep`,
stage `sweep`, over `tests/unit/sweep/`, after the `unit` entry, and the
directory holds three members, each declaring its own domain in
`// sweep-count:` and printing its own `swept <N>` (TM-122):

| Member | Domain | Asserts |
|---|---|---|
| `every_day_number.npk` | 7 304 484 day numbers | each to its date and back — `CALENDAR.md` C-16's first half |
| `every_civil_date.npk` | 7 304 484 dates, generated by the leap rule and the month table | each to its day number and back — C-16's second half — and each number the previous one's plus one (C-17's first rider, TM-167) |
| `every_month_length.npk` | 239 988 months | `days_in_month` against the distance between consecutive month-firsts — C-17's third rider (TM-167) |

**Each loop is a `for` over a range**: `..` is inclusive at both ends, and a
`for` is bounded by construction and states no measure (the compiler's
CONTROL_REFERENCE §2). So each member owes exactly the eleven identities any
program importing `cal` owes — measured at execution, `NITPICK-REACH-003` on a
copy of each with no `failsafe` lists `cal.ETimeValue` and the ten of
`SAFETY.md` S-4's `cal` row: no `DecreasesViolated`, no `BadStep`. **The print
helper is ten lines, one copy per file** — `int_to_string`, `string_concat`,
`to_cstring`, and `sys` for `write` — and the harness reads what each copy
prints on every run.

*Declined:* one member printing a combined count — V-16 is one `swept` line per
member and a count names a domain, and a combined 14 848 956 could not say
...
harness checks on every run; `while` loops — a measure each and
`DecreasesViolated` in every sweep's bill, for nothing a `for` over a range
does not give.

### TM-167 — C-17's riders are stated in the form a wrong implementation fails

**2026-09-25, cycle 0.1.2 (PD-18). Amends `CALENDAR.md` C-17**, whose three
items as first written are quoted verbatim in the dated note beside the new
text. Each of the three, as first written, is passed by a mutant — measured —
so each is restated in the form that mutant fails:

1. **Monotonicity: each date's day number is the previous one's plus one.**
   *"Strictly increasing"* passes a leap rule missing its 400-year day. On that
   mutant (M2 of `0.1.2.md` §5) the civil walk with a `>` chain exits 0 having
   printed `swept 7304435` — the 49 February 29ths of the multiples of 400 from
   −9600 to 9600 were never visited, every date that was still round-trips, and
   the last number is still `NTIME_DAY_MAX` — so only the harness's count would
   have caught it. The `+ 1` chain exits 13.
2. **The weekday cycle: each day's weekday equals a count begun at
   −9999-01-01's Monday and advanced by one per day**, so it is in Monday …
   Sunday on every day of the range — asserted from 0.1.3 (TM-165).
   *"Advances by one, mod seven"* passes a weekday computed as `(n + 3) % 7`
   with the language's truncating `%` and no correction (probe07): re-derived
   at execution in Python over the whole range, that weekday passes the
   advance check on all 7 304 483 steps and fails the count on the range's
   second day; it is wrong on six days in seven before 1969-12-29.
3. **Month lengths: `days_in_month` equals the distance from this month's first
   day to the next month's** (the last month's to `NTIME_DAY_MAX + 1`).
   *"Last less first plus one"* builds the month's last day with the length
   under test and counts to it, and `date_to_days` is total (C-12, TM-162), so a
   February 29th that should not exist gets the next day's number. That
   formulation exits 0 with `swept 239988` on the century-rule, 400-year-rule
   and April-31 mutants (M1, M2, M8); the chosen one exits 12 on all three.

*Declined:* keeping C-17's wording and strengthening only the tests — a rule
weaker than its checks lets the next test be written to the rule; a new rule
beside C-17 — C-17 is where a reader looks.

### TM-168 — a `sweep` member's declared domain is a measured denominator, `domain_<stem>`, and every live statement of it is tagged

**2026-09-25, cycle 0.1.2 (PD-19). Extends `TESTING.md` V-1h.** `harness/checks.py`'s
`denominators()` reads each member of `tests/unit/sweep/` through
`stages.read` — the parser the suite dispatches on — and measures its
`// sweep-count:` as `domain_<stem>`: at this commit `domain_every_day_number`
and `domain_every_civil_date` 7 304 484, `domain_every_month_length` 239 988.
Eleven tags carry them: eight at the live statements of a domain
(`CALENDAR.md` §2's total and C-16's count, `TESTING.md` V-2's two sizes,
`TIME_MODEL.md`'s range row, `src/cal/README.md`, `harness/stages.py`'s
`sweep()` docstring and `src/core/limits.npk`'s header relation) and three
beside the members in `nitpick.toml`'s sweep block. So the specification's
number and the test's are one list, diffed on every run, where TM-161's were
two; and since the `sweep` stage holds each program's printed count to its
header (TM-122), a green run closes the chain *specification = header = what
the program visited* — the civil walk's count being itself a recomputation of
the range's size from the leap rule and the month table.

**Commissioned before it was trusted** (TM-126): `harness/selfcheck.py`'s
`PLANTED` gained a row — a member declaring 10 beside a tag saying 11 —
seen RED before the measurement existed (2 problems: the needle missing, and
the clean control firing) and silent after (0). The self-check reads **21**
planted violations and 21 controls.

*Declined:* a one-time diff at execution only — that is what `limits.npk`'s
three hand-checked relations were, and they held a wrong first day for three
weeks; the harness recomputing the calendar's size in Python on every run — a
third implementation of C-3 inside the harness and a map from file names to
domains, when the civil walk's own count already recomputes the day total from
the leap rule on every run; tagging roadmap files — they are archived at the
