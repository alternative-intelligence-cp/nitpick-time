...
- [x] **CI IS GREEN. Run [`34015036991`](https://github.com/alternative-intelligence-cp/nitpick-time/actions/runs/34015036991) on a push of `main` at `8081e60`: conclusion `success`, job `build`, 8m02s, ALL ELEVEN declared steps green** — including *Build the compiler from the pinned commit, FROM ITS TREE ROOT*, which W-18 forbids rehearsing from here. **The acceptance item cycle 0.0 has carried since 0.0.1 is met.** `GREEN -- 62 unit(s), 0 failures; 5 pending, 49.7 s` — the same 62 units and the same verdicts as the workbench, and *faster* (49.7 s against 62.4 s). Three claims this cycle made are confirmed in the one environment that could test them: `ok llc 20.1.2 **LLVM version 20.1.2**` is the UPSTREAM tarball's banner with the number on **line two**, which the pre-0.0.6 first-line match would have refused (TM-140), and `ld.lld` printed a THIRD banner shape nobody had seen; `1 nested repository pruned: .nitpick` is TM-146's fix, reported rather than silent; and `78 .npk in the tree = 10 src/ + 68 tests/ + 0 elsewhere` and `defect corpus: 24 = 3 exempt + 21 asserted` are identical to the workbench's, which is what makes the denominators a property of the tree rather than of this laptop. *(The RED first run and what it bought follow.)* **CI'S FIRST RUN WENT RED — WHICH WAS THE FINDING (TM-146).** Run [`34014136095`](https://github.com/alternative-intelligence-cp/nitpick-time/actions/runs/34014136095) on a push of `main` at `f950ae4`, cycle 0.0.6's own close commit: conclusion **`failure`**, job `build`, **13m37s**. Read back off the run with `gh`, not accepted from a report. **Ten of the eleven declared steps are `success`**, including the two that cannot be rehearsed from this machine — *Build the compiler from the pinned commit, FROM ITS TREE ROOT* (`success`, not `skipped`: the cache was cold on a first run, so the compiler genuinely built through `quickemit.py`) and *Put LLVM on PATH and assert the exact patch release*. **Step 11, `Run the harness`, is the failure**, and it found two defects nothing on the workbench could — the whole-tree walk enters the compiler checkout CI makes at `.nitpick`, and the step's own log capture never ran because GitHub's default shell is `bash -e`. Both fixed at TM-146. **And one measurement worth more than the red:** `npkrt.o` is byte-identical across machines (`c9ddbcff…` both), while `npkc` is not (`3c05818c…` on CI against `a3b0dadc…` here) — different recipes, `quickemit.py` against `npkg build`, exactly as the workflow's digest step predicted. *(Original item follows.)* CI: a workflow running `harness/run.py` on push, with LLVM 20.1.2 and the compiler built from a **pinned commit** — **written and validated locally, NOT YET SEEN GREEN.** `.github/workflows/ci.yml` pins compiler `0dfddac045bdab6abbd367b1ffb31de695b9bf22` and LLVM `20.1.2`, asserts both plus a clean checkout and the ladder's working directory, and reports the artefact digests against the workbench's rather than asserting cross-machine byte identity nobody has measured. The YAML parses and all six `run:` blocks pass `bash -n`. **The acceptance item says "green on a push" and 0.0.1 does not push** (`ROADMAP.md`: push at the end of a cycle), so this carries to the push that closes 0.0
