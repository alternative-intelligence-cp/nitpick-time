...
| Stage | Answers |
|---|---|
| `parse` | every source in the tree is readable by the real parser — the grammar is never quietly made partial. **A whole-tree stage, not a `[[test]]` entry**, and it asks `$NPKC` rather than the compiler's `tools/parse_check`: TM-123 has the measurement and the reason. Its value here is the **29 files of 83 that no other stage roots** <!-- [[sweep: npk_total=83]] --> |
...
| `check_civil_literal` (TM-148) | **no `CivilDate{` or `CivilTime{` struct literal in `src/` outside `src/cal/cal.npk`.** C-8 says there is no unchecked constructor; measured at pin `aaffb87`, a consumer's `CivilDate{ year: 32000i32, month: 99u8, day: 99u8 }` compiles, links and **runs at exit 0** — the language has no private field and `opaque struct:Name = { … };` is refused, so the literal cannot be taken away (C-8b). The check covers the half that is enforceable, which is the library itself: both use-after-frees cycle 0.0 shipped were `ntime` defeating its own stated contract under a green suite. **Its clean control is the banned form in a COMMENT**, because `src/lib.npk`'s header spells the fake-date literal out in prose to document this rule — a grep-shaped version fails the repository on the paragraph arguing for it |
