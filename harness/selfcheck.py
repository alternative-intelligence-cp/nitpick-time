...
def part_b_specs_current(rep, base):
    ...
    _write(os.path.join(where, "meta", "DECISIONS.md"),
           "# Decisions\n\n### TM-100 - a decision that exists\n")
    _write(os.path.join(where, "meta", "specs", "SAFETY.md"),
           "# Safety\n\n**Rule S-1.** A rule that exists.\n")
    _write(os.path.join(where, "CLAUDE.md"),
           "This cites TM-100, which resolves, and TM-999, which does not.\n"
           "It cites S-1, which resolves, and S-77, which does not.\n")
...
def run(rep, root, steps):
    ...
    rep.say("[1/%d] self-check -- V-14's 8 cases, then %d tree checks on "
            "planted violations," % (steps, len(PLANTED) + 1))
    ...
    if problems:
        ...
        rep.say("  ok    %-46s %s" % ("the tree checks",
                                      "%d planted violation(s) caught, %d clean "
                                      "control(s) silent"
                                      % (len(PLANTED), len(PLANTED) + 1)))
