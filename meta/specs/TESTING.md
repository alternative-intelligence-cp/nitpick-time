...
*(**And it was false again from cycle 0.1.0 to 0.1.0b, and nobody moved it.**
Cycle 0.1.0 added `check_civil_literal` (TM-148) as an eighteenth row and a
fourteenth live check, and this rule went on saying 17 and 13. **Cycle 0.1.0c
retired that check** (TM-158) — `NITPICK-TYPE-079` now refuses a `CivilDate{`
or `CivilTime{` literal in every module but `cal`, consumers included, which
is exactly the property the check held `src/` to — so its row is gone and the
arithmetic above is true again, re-derived rather than inherited: `checks.LIVE`
holds 9, `run.py`'s `[5/9]` line reads `10 live` (the 9 and
`check_failsafe_arms`), and the three driven outside step 5 make 13. The row
