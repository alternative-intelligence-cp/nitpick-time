...
| 0.1.3c | **`Vec` move-only by construction** — question 9, answered 2026-09-25 — porting `nitpick-regex`'s 0.0.4d and 0.0.4e, both landed and verified: the marker field; `vec_at<T: Pod>` through a `Pod` trait and a LOAN, which the marker forces at this pin, and `generic_owning_copy/case5` refused `NITPICK-TYPE-017`; `Bytes.body` hidden, which question 9's answer let ride; TM-150's churn pair committed with `heap:` bounds; A′ as `VERIFICATION.md` rule P-1b, striking Q-6; `Pod` shaped for the prelude's `Copy` to replace at a later re-pin; and the reason-sweep over the six defects the registry struck at `c970483` — **[`0.1.3c.md`](0.1.3c.md), PLANNED 2026-09-26 against compiler `c970483`; it runs after 0.1.4c** (the orchestrator, 2026-09-26) | no copy of a `Vec` and no destructive `vec_at` compiles (PD-44, PD-45); `GREEN -- 112 unit(s)` |
...
- [ ] `Vec` move-only by construction, porting `nitpick-regex`'s 0.0.4d: `hidden string[0]:move_only`, and a copy of a `Vec`, an assignment of one over another and a copy of a struct holding one each refused `NITPICK-TYPE-046` — each run to the free poison against the tree before, and against an `int64[0]` marker (PD-44)
- [ ] `vec_at<T: Pod>` through a loan, porting its 0.0.4e — `Pod` and its nine impls exactly `nitpick-regex`'s; `vec_pop` already spells `move`; and `vec_at`'s `#wild_slice` read at an owning `T`, which DEF-104's gate does not reach (`0.1.4c.md` §1.3, §11), settled: `case5` refused `NITPICK-TYPE-017`, its three worlds in its transcript (PD-45)
- [ ] `Bytes.body` hidden and `bytes_capacity` — question 9's answer, judged cheaper here (PD-46)
- [ ] **from 0.1.4b's planning:** TM-150's `vec_pop` churn pair — two million push-then-pop cycles, `peak_live` 120 against `vec_clear`'s 48 000 096 — committed as two programs with `heap:` bounds and the cap, every bound seen red on its mutant (PD-47; `0.1.4b.md` §15)
- [ ] A′ as `VERIFICATION.md` rule P-1b, and Q-6 struck with its number (PD-48)
- [ ] `Pod` shaped so the prelude's `Copy` — the compiler's D-327, ratified during this planning and in no pin of ours — replaces it mechanically at the re-pin that carries it, here and in `nitpick-regex`; nothing named `Copy` (PD-49)
